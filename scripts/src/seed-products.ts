...
async function seed() {
  console.log("Cleaning men's categories and products if any...");

  // Remove any men's categories that may have been added
  const menCatNames = ["Men's Shirts", "Men's Pants", "Men's T-Shirts", "Men's Jackets", "Shoes"];
  const menCats = await db.select().from(categoriesTable);
  const menCatIds = menCats.filter(c => menCatNames.includes(c.nameEn)).map(c => c.id);
  if (menCatIds.length > 0) {
    await db.delete(productsTable).where(inArray(productsTable.categoryId, menCatIds));
    await db.delete(categoriesTable).where(inArray(categoriesTable.id, menCatIds));
    console.log("Removed men's categories:", menCatIds);
  }

  // Add women/girls categories if they don't exist
  const existingCats = await db.select().from(categoriesTable);
  const existingNames = existingCats.map(c => c.nameEn);
  ...
  ].filter(c => !existingNames.includes(c.nameEn));

  if (catsToAdd.length > 0) {
    const added = await db.insert(categoriesTable).values(catsToAdd).returning();
    added.forEach(c => { catMap[c.nameEn] = c.id; });
  }
  // Refresh map with existing
  const allCats = await db.select().from(categoriesTable);
  allCats.forEach(c => { catMap[c.nameEn] = c.id; });
  console.log("Categories:", Object.keys(catMap));

  // Modest women images from Unsplash
  const products = [
    // ===== DRESSES =====
    {
      nameAr: "فستان ماكسي بوهيمي",
      nameEn: "Bohemian Maxi Dress",
    ...
    {
      nameAr: "فستان فلوري ربيعي",
      nameEn: "Floral Spring Dress",
    ...
    {
      nameAr: "فستان محتشم بأكمام طويلة",
      nameEn: "Modest Long-Sleeve Dress",
    ...
    {
      nameAr: "فستان حفلات شيفون",
      nameEn: "Chiffon Party Dress",
    ...
    // ===== TOPS =====
    {
      nameAr: "بلوزة شيفون محتشمة",
      nameEn: "Modest Chiffon Blouse",
    ...
    {
      nameAr: "توب ساتان لامع",
      nameEn: "Satin Shimmer Top",
    ...
    {
      nameAr: "بلوزة كاجوال يومية",
      nameEn: "Casual Daily Blouse",
    ...
    // ===== PANTS =====
    {
      nameAr: "بنطلون كاريه كلاسيك",
      nameEn: "Classic Straight-Cut Pants",
    ...
    {
      nameAr: "بنطلون واسع بوهيمي",
      nameEn: "Wide-Leg Boho Pants",
    ...
    // ===== ABAYAS =====
    {
      nameAr: "عباية خليجية بتطريز ذهبي",
      nameEn: "Gulf Abaya with Gold Embroidery",
    ...
    {
      nameAr: "عباية كريب مودرن",
      nameEn: "Modern Crepe Abaya",
    ...
    {
      nameAr: "عباية جورجيت مطرزة",
      nameEn: "Embroidered Georgette Abaya",
    ...
    {
      nameAr: "شنطة كتف أنيقة",
      nameEn: "Elegant Shoulder Bag",
    ...
    {
      nameAr: "كلتش سهرة لامع",
      nameEn: "Glitter Evening Clutch",
    ...
    // ===== KIDS' CLOTHES (Girls) =====
    {
      nameAr: "فستان بنات ورد جميل",
      nameEn: "Floral Girls' Dress",
    ...
    {
      nameAr: "طقم بنات كاجوال",
      nameEn: "Girls' Casual Set",
    ...
    {
      nameAr: "فستان عيد فاخر للبنات",
      nameEn: "Luxury Girls' Eid Dress",
    ...
    {
      nameAr: "بيجاما بنات قطن ناعم",
      nameEn: "Soft Cotton Girls' Pajama",
    ...
    {
      nameAr: "طقم مدرسي بنات",
      nameEn: "Girls' School Outfit",
    ...
    {
      nameAr: "فستان طرحة للبنات",
      nameEn: "Girls' Hijab Dress Set",
    ...
    // ===== ACCESSORIES =====
    {
      nameAr: "ساعة نسائية أنيقة",
      nameEn: "Elegant Women's Watch",
    ...
    {
      nameAr: "نظارة شمسية نسائية",
      nameEn: "Women's Fashion Sunglasses",
  ...
  for (const p of products) {
    if (!p.categoryId) { console.warn("Skipping (no category):", p.nameEn); continue; }
